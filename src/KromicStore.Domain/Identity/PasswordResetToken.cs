...
public sealed class PasswordResetToken : BaseEntity
{
    ...
    private PasswordResetToken(Guid id, Guid userId, string tokenHash, DateTime expiresOnUtc) : base(id)
    {
        UserId = userId;
        TokenHash = tokenHash;
    ...
    public static PasswordResetToken Create(Guid userId, string tokenHash, DateTime expiresOnUtc)
    {
        ...
        return new PasswordResetToken(Guid.NewGuid(), userId, tokenHash, expiresOnUtc);
    }
